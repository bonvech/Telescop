        ...
        for i in range(len(d)):
            if d[i][0] in '0123456789':
                if int(d[i].split()[0]) == int(ev):
                    ...
                    if not b.f:
                        b.win.append(Ustanovka(b, self, s))
                        b.win[-1].show()
                        b.f = False
                    return None
...
class WorkerPD(QObject):
    ...
    def __init__(self, parent):
        ...
        self.parent = parent
        self._step = int(self.parent.sq.value())
...
class Ustanovka(QMainWindow):
    ...
    def __init__(self, b, parent, s):
        ...
        self.a1 = list(map(int, s[0][1:]))
        self.a2 = list(map(int, s[1][1:]))
        self.a3 = list(map(int, s[2][1:]))
        self.a4 = list(map(int, s[3][1:]))

        self.d1 = list(map(int, s[4][1:]))
        self.d2 = list(map(int, s[5][1:]))
        self.d3 = list(map(int, s[6][1:]))
        self.d4 = list(map(int, s[7][1:]))

        self.tr1 = list(map(int, s[8][1:]))
