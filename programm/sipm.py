...
class App(QMainWindow):
    ...
    def openEventInMAatplot(self):
        ...
        for sp in self.coords['SIPM'].unique().tolist():

            for i in self.coords[self.coords['SIPM'] == sp]['ch'].unique().tolist():
                sipm.append(sp)
                ch.append(i)
                znach.append(max(self.df[(self.df['SIPM'] == sp) & (self.df['ch'] == i) & (self.df['Event'] == int(self.event))].iloc[0, 5+self.stbord:5+self.labord]))
                X.append(self.coords[(self.coords['SIPM'] == sp) & (self.coords['ch'] == int(i))]['x_sm'].unique().tolist()[0])
                Y.append(self.coords[(self.coords['SIPM'] == sp) & (self.coords['ch'] == int(i))]['y_sm'].unique().tolist()[0])
